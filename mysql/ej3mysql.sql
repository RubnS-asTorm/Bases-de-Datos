...
##
SELECT COUNT(*), COUNT(comision), COUNT(oficio), COUNT(DISTINCT oficio) 
FROM empleados;

#5
select count(comision is null)
from empleados
...
#8
select min(salario) as "salario mas bajo", max(salario) as "salario mas alto" , avg(salario) as "salario de media"
from empleados;
...
#10
select avg(ifnull(salario,0)+ifnull(comision,0))
from empleados
where oficio = "vendedor";

#11
select avg(precio_actual * unidades)
from productos inner join pedidos
on productos.PRODUCTO_NO = pedidos.producto_no;

#12
select max(precio_actual * unidades)
from productos inner join pedidos
on productos.PRODUCTO_NO = pedidos.producto_no;
...
from empleados
where not (comision is NULL or comision like "0%");

#15
select count(*) as "Número de empleados que ganan más de comisión que de salario"
from empleados
where comision > salario;
select count(*) as "Número de empleados que ganan más de salario que de comisión"
from empleados
where comision < salario;

#16
select max(precio_actual) as "producto más caro del que dispone la empresa"
from productos;

#17
select max(precio_actual) as "producto más caro"
...
#22
select sum(salario) as "salarios empresa", sum(comision) as "comisiones empresa", sum(ifnull(salario,0)+ifnull(comision,0)) as "gasto total"
from empleados;
...
#25
select sum(precio_actual * unidades)
from productos inner join pedidos
on productos.producto_no = pedidos.producto_no
where fecha_pedido between "1999-10-01" and "1999-11-01";
...
from productos inner join pedidos
on productos.producto_no = pedidos.producto_no
where fecha_pedido between "1999-06-01" and "2000-05-01";
